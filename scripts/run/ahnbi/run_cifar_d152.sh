#!/bin/bash
# ahnbi/run_cifar_d152.sh — Exp 1 잔여: Depth 152 (8 runs)
# RTX 3090, ~2.5h/run → ~20h
#
# 실행:
#   nohup bash scripts/run/ahnbi/run_cifar_d152.sh 0 > logs/ahnbi_d152.log 2>&1 &
set -e
...
LOG_DIR=logs
mkdir -p $LOG_DIR
BATCH_LOG=$LOG_DIR/ahnbi_d152.log

echo "=== [ahnbi] Depth152 Start: $(date) | GPU: $GPU_ID ===" | tee -a $BATCH_LOG
...
for dataset in cifar10 cifar100; do
  for seed in 123 456; do
    run_one lrs_mid 152 "$dataset" "$seed"
  done
done

# lrs_high d152 (4 runs)
for dataset in cifar10 cifar100; do
  for seed in 123 456; do
    run_one lrs_high 152 "$dataset" "$seed"
  done
done

echo "=== [ahnbi] Depth152 Complete: $(date) ===" | tee -a $BATCH_LOG
