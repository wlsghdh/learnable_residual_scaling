#!/bin/bash
# ahnbi/run_cifar_d101.sh — Exp 1 잔여: Depth 101 (9 runs)
# RTX 3090, ~1.5h/run → ~14h
#
# 실행:
#   nohup bash scripts/run/ahnbi/run_cifar_d101.sh 0 > logs/ahnbi_d101.log 2>&1 &
set -e

GPU_ID=${1:-0}
export CUDA_VISIBLE_DEVICES=$GPU_ID

cd "$(dirname "$0")/../../.."
LOG_DIR=logs
mkdir -p $LOG_DIR
BATCH_LOG=$LOG_DIR/ahnbi_d101.log

echo "=== [ahnbi] Depth101 Start: $(date) | GPU: $GPU_ID ===" | tee -a $BATCH_LOG

run_one() {
  local model=$1 depth=$2 dataset=$3 seed=$4
  RESULT_FILE="results-json/${model}_depth${depth}_${dataset}_seed${seed}_result.json"
  if [ -f "$RESULT_FILE" ]; then
    echo "[$(date)] SKIP (exists): $model d$depth $dataset s$seed" | tee -a $BATCH_LOG
    return
  fi
  echo "[$(date)] Running: $model d$depth $dataset s$seed" | tee -a $BATCH_LOG
  python run_experiments.py \
    --model "$model" --depth "$depth" --dataset "$dataset" --seed "$seed" \
    >> $LOG_DIR/${model}_d${depth}_${dataset}_s${seed}.log 2>&1
  echo "[$(date)] Done:    $model d$depth $dataset s$seed" | tee -a $BATCH_LOG
  sleep 30
}

# lrs_mid d101 cifar100 s456 (중단되었던 것)
run_one lrs_mid 101 cifar100 456

# lrs_high d101 (4 runs)
for dataset in cifar10 cifar100; do
  for seed in 123 456; do
    run_one lrs_high 101 "$dataset" "$seed"
  done
done

# rezero d101 (4 runs)
for dataset in cifar10 cifar100; do
  for seed in 123 456; do
    run_one rezero 101 "$dataset" "$seed"
  done
done

echo "=== [ahnbi] Depth101 Complete: $(date) ===" | tee -a $BATCH_LOG
