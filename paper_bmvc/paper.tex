...
% Note: bmvc2k.cls already loads hyperref with colored links (URL=blue, cite=red)
% — this is BMVC default; do not redefine here

% Float spacing — prevent tables/figures from overlapping body text
...
% Float placement: discourage LaTeX from deferring floats to end of doc
...
% Force floats to appear before next section
...
ResNet~\cite{he2016deep} introduced skip connections to alleviate gradient vanishing in deep networks; identity mappings further improve gradient flow~\cite{he2016identity}. Highway Networks~\cite{srivastava2015highway} preceded ResNet with gated information flow. De \& Smith~\cite{de2020batch} showed batch normalization biases standard ResNet residuals toward identity---a phenomenon LRS makes explicit but does not create.
...
Using $\alpha > 0.3$ as ``active block'' threshold, only 5--6 of 66 blocks in a 200-layer network qualify---effective depth $\sim$8\% of nominal. To avoid threshold dependence, we additionally report the threshold-free metric $D_\text{eff} = \sum_i \alpha_i$ (Table~\ref{tab:deff}): quadrupling blocks ($16 \to 66$) increases $D_\text{eff}$ by only $40\%$ ($5.4 \to 7.5$). Per-block decomposition shows that within-stage $\bar{\alpha}$ stays in $[0.06, 0.15]$ (near-identity) while downsampling-block $\bar{\alpha}$ remains in $[0.42, 0.96]$ (active transformation).

\begin{table}[!ht]
\centering
\small
\caption{Threshold-free effective depth $D_\text{eff} = \sum_i \alpha_i$ for LRS Low on CIFAR-100. Although nominal depth scales 4$\times$, $D_\text{eff}$ scales only 1.4$\times$, confirming that most added blocks are not used.}
\label{tab:deff}
\begin{tabular}{lcccc}
\toprule
\textbf{Depth} & \textbf{Blocks} ($N$) & $D_\text{eff} = \sum \alpha_i$ & $D_\text{eff}/N$ & \textbf{Active} ($\alpha{>}0.3$) \\
\midrule
50  & 16 & 5.4 & 34.0\% & 6--7 \\
101 & 33 & 6.0 & 18.3\% & 6 \\
152 & 50 & 6.7 & 13.3\% & 5--6 \\
...
If the optimal $\alpha$ is much less than 0.5, why does standard ResNet (implicit $\alpha = 0.5$) perform well? Standard ResNet uses $y = F(x) + x$, which does not constrain $\|F(x)\|/\|x\|$. Through batch normalization and weight magnitude adjustment, the network can effectively achieve a small ratio without an explicit $\alpha$---a phenomenon De \& Smith~\cite{de2020batch} established independently. LRS makes this implicit ratio observable and quantifies it.

\subsection{Practical Implications}
\label{sec:practical_implications}

Our findings translate into three actionable guidelines. \textbf{(1) $\alpha$-guided pruning} provides an automatic block-importance criterion produced as a training byproduct---one-shot workflow (train once with LRS, threshold $\alpha$, fine-tune $\sim$10 epochs) yields $54\%$ FLOPs / $52\%$ latency reduction at $4.6\%$ accuracy cost. \textbf{(2) Identity-favoring initialization} ($\alpha_0 \leq 0.12$, $\theta_0 \leq -2$) is necessary at extreme depth---residual-favoring initialization actively destabilizes training. \textbf{(3) Architecture design}: the heterogeneous $\alpha$ distribution justifies allocating capacity at stage boundaries rather than within stages, and NAS procedures could use $\alpha$-statistics as a regularizer against over-deep within-stage configurations.

\subsection{Limitations}

\textbf{ImageNet scale.} We verify the scaling law at ImageNet depths 50 and 101 with single seeds; deeper models and additional seeds would strengthen the claim. The CIFAR seed-noise floor ($<1\%$ of the mean) and the 20--80$\times$ larger ImageNet-CIFAR gaps argue against seed accidents, but ImageNet's internal noise is not directly characterized. \textbf{Measurement bias.} LRS adds one parameter per block; identity-init experiments (Section~\ref{sec:identity_init}) show convergent $\bar{\alpha}$ regardless of starting point, arguing intrinsic convergence. \textbf{Architecture scope.} The analysis is restricted to bottleneck ResNets; basic blocks and Vision Transformers are not covered. \textbf{Threshold dependency.} The ``active block'' threshold $\alpha > 0.3$ is arbitrary, addressed via the threshold-free $D_\text{eff} = \sum_i \alpha_i$.

%-------------------------------------------------------------------------
\section{Conclusion}
\label{sec:conclusion}

We proposed and empirically supported a concrete answer to why deep ResNets train successfully: they autonomously select an effective depth far smaller than their nominal depth, and skip connections provide the architectural freedom that makes this self-selection possible. Across $120$ CIFAR runs and ImageNet at depths 50/101, deep ResNets compress effective depth to roughly $8\%$ of nominal; the depth--$\bar{\alpha}$ scaling law is consistent across datasets. Skip connections are the enabling mechanism: without them gradients explode and self-selection is impossible. The selection is intrinsic, validated by random/identity-init invariance ($\bar{\alpha}$ within $4\%$) and by $\alpha$-guided pruning---$\alpha < 0.03$ blocks remove with zero accuracy loss, and $56\%$ block pruning yields $54\%$ FLOPs / $52\%$ latency reduction at $4.6\%$ accuracy cost. The conventional view casts deep networks as ``working'' only when each added layer contributes positively. Our results suggest a different reading: deep networks work when they are allowed not to use most layers. Capacity not used is not wasted---it is reserve, and skip connections make reserve a viable design choice.
